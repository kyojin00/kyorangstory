...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { data: comment } = await supabase
    .from('story_comments').select('user_id').eq('id', commentId).maybeSingle();
  ...
  const { data, error } = await supabase
    .from('story_comments').update({ content: content.trim() }).eq('id', commentId).select().single();
...
export async function DELETE(_: NextRequest, { params }: Params) {
  ...
  await supabase.rpc('decrement_story_comments_count', { story_id: storyId });
