    ...
    <div style={{ display: 'grid', gridTemplateColumns: count === 1 ? '1fr' : count === 3 ? '2fr 1fr' : 'repeat(2, 1fr)', gap: '3px', borderRadius: '12px', overflow: 'hidden', marginBottom: '12px' }}>
      {images.slice(0, 4).map((url, i) => (
        <div key={url} style={{ aspectRatio: count === 1 ? '16/9' : '1', gridRow: count === 3 && i === 0 ? 'span 2' : 'auto', position: 'relative' }}>
          ...
          {i === 3 && count > 4 && (
            <div style={{ position: 'absolute', inset: 0, background: '#000a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.5rem', fontWeight: 800 }}>+{count - 4}</div>
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a0a2e, #0f0820)', border: '1px solid #4c1d9555', borderRadius: '16px', padding: '18px' }}>
        <p style={{ margin: '0 0 6px', fontSize: '1rem', fontWeight: 800, color: '#a78bfa' }}>🌙 익명 스토리</p>
...
export default function FeedPage() {
  ...
  const handleTabChange = (tab: FeedTab) => { setFeedTab(tab); fetchPosts(tab); };
    ...
    <SidebarLayout rightPanel={<RightPanel />}>
      ...
      {editTarget   && (
        <EditPostModal
          ...
          onSave={newContent => {
            setPosts(prev => prev.map(p => p.id === editTarget.id ? { ...p, content: newContent } : p));
          }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button onClick={handlePost} disabled={(!content.trim() && !images.length) || posting}
                        style={{ background: (content.trim() || images.length) && !posting ? '#7c3aed' : '#1e1b3a', border: 'none', borderRadius: '20px', padding: '6px 20px', color: (content.trim() || images.length) && !posting ? '#fff' : '#475569', fontSize: '0.875rem', fontWeight: 700, cursor: (content.trim() || images.length) && !posting ? 'pointer' : 'not-allowed' }}>
                        {posting ? '게시 중...' : '게시'}
                      </button>
                    </div>
                  </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      <button onClick={() => setMenuOpen(prev => prev === post.id ? null : post.id)}
                        style={{ background: 'transparent', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '1rem', padding: '2px 6px', borderRadius: '6px' }}>···</button>
                      {menuOpen === post.id && (
                        <div style={{ position: 'absolute', right: 0, top: '28px', background: '#0f0f1f', border: '1px solid #1e1b3a', borderRadius: '12px', padding: '6px', zIndex: 50, minWidth: '140px', boxShadow: '0 8px 24px #0008' }}>
                          {isMyPost ? (
                            <>
                              <button onClick={() => { setEditTarget(post); setMenuOpen(null); }}
                            ...
                            <>
                              <button onClick={() => { setReportTarget(post.id); setMenuOpen(null); }}
                                style={{ width: '100%', padding: '8px 12px', background: 'transparent', border: 'none', color: '#f97316', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}
                                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#1e1b3a'; }}
                                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
                                🚨 신고하기
                              </button>
                              {username && (
                                <button onClick={() => handleBlock(username)}
                                  style={{ width: '100%', padding: '8px 12px', background: 'transparent', border: 'none', color: '#ef4444', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}
                                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#1e1b3a'; }}
                                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
                                  🚫 차단하기
                          ...
                          <button onClick={() => { navigator.clipboard.writeText(`${location.origin}/feed/${post.id}`); setMenuOpen(null); }}
                            style={{ width: '100%', padding: '8px 12px', background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}
                            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#1e1b3a'; }}
                            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
                            🔗 링크 복사
                          </button>
